...
import { Share2, Bookmark, Check, LogIn } from 'lucide-react'
...
export default function ShareSaveButtons({ id, title, type }: ShareSaveButtonsProps) {
  ...
  return (
    <div className="flex gap-3">
      ...
      >
        {copied ? <Check size={18} className="text-green-500" /> : <Share2 size={18} className="group-hover:text-blue-500" />}
        <span className="hidden md:inline">{copied ? '링크 복사됨' : '공유하기'}</span>
      ...
      >
        ...
        <span className="hidden md:inline">{isSaved ? '저장됨' : '저장하기'}</span>
      </button>
    </div>
  )
}
