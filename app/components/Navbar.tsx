					...
					<div className="flex items-center justify-between">
							...
							<nav className="hidden lg:flex items-center gap-6">
								{NAV_LINKS.map((link) => (
									<NavLink
										...
										active={
											pathname === link.href ||
											(link.href === "/" && pathname.includes("/deposits"))
										}
									>
										{link.label}
									</NavLink>
						...
						<div className="flex items-center gap-2 md:gap-4">
							...
							<div className="flex items-center gap-2">
								<button
									type="button"
									className="hidden sm:flex p-2.5 bg-white border border-gray-100 text-gray-400 hover:text-gray-900 rounded-xl shadow-sm transition-all"
								>
									<Search size={18} />
								</button>
						...
						>
							<div className="p-6 pb-4 flex items-center justify-between border-b border-gray-100">
								...
								<button
									type="button"
									onClick={() => setIsOpen(false)}
									className="p-2.5 bg-white border border-gray-100 text-gray-400 rounded-xl shadow-sm"
								>
									<X size={20} />
								</button>
							</div>
							...
							<div className="flex-1 overflow-y-auto p-6 space-y-8">
								<div className="space-y-3">
									<p className="px-2 text-[10px] font-black text-gray-400 uppercase tracking-widest opacity-60">
										Portfolio
									</p>
									<SidebarLink
										href="/saved"
...
function NavLink({
	...
	return (
		...
		>
			{children}
			<span
				className={`absolute -bottom-1 left-0 h-[2.5px] bg-gray-900 transition-all ${
		...
		<Link
			...
			className={`flex items-center gap-4 p-4 rounded-[20px] transition-all border shadow-sm ${
				active
					...
					: highlight
						? "bg-gray-900 text-white font-black border-gray-900 shadow-xl shadow-gray-900/20"
						: "bg-white border-gray-50 text-gray-600 font-bold hover:bg-gray-50"
			}`}
		>
			<div
				className={`${active ? "text-blue-600" : highlight ? "text-white" : "text-gray-400"}`}
			>
				{icon}
			</div>
