...
import { getLoans, type LoanProduct } from "@/app/actions/loan";
...
export default function LoansPage() {
	const [activeTab, setActiveTab] = useState<"credit" | "mortgage" | "rent">(
		"credit",
	);
	...
	useEffect(() => {
		async function load() {
			setIsLoading(true);
			const { products: data, isMock: mockStatus } = await getLoans(
				activeTab,
				selectedFilters,
				tier,
				selectedBanks,
			...
			>
				<div className="container mx-auto flex items-center justify-center gap-3">
					<p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
						{isMock
							? "Simulation Mode: 데이터 분석 중"
							: "Live Data: 실시간 금리 연동 중"}
					</p>
				</div>
			</div>
						...
						<div className="flex flex-wrap p-2 bg-white rounded-[32px] border border-gray-100 shadow-sm w-fit gap-2">
							{tabs.map((tab) => (
								<button
									key={tab.id}
									onClick={() =>
										setActiveTab(tab.id as "credit" | "mortgage" | "rent")
									}
