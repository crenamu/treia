	...
	return (
		<PremiumProductTemplate
			...
			primaryRate={{
				...
				subSuffix: product.loan_lmt.includes("억")
					? "억원"
					: product.loan_lmt.includes("만")
						? "만원"
						: "",
			}}
			tags={product.tags}
			metrics={[
				...
				{
					label: "방식",
					value: product.bestOption?.rpay_type_nm || "원리금균등",
					icon: <Wallet size={14} />,
				},
			]}
			...
			details={[
				{
					label: "금리 상세",
					value: `최저 ${product.bestOption?.lend_rate_min}% ~ 최고 ${product.bestOption?.lend_rate_max}% (평균 ${product.bestOption?.lend_rate_avg}%)`,
				},
				...
				{ label: "상환 방식", value: product.bestOption?.rpay_type_nm || "-" },
				{
					label: "유의사항",
					value:
						"대출 금리는 신용점수 및 은행 내부 심사 기준에 따라 차등 적용될 수 있습니다.",
					isText: true,
				},
			]}
...
function LoanRepaymentSimulator({ product }: { product: LoanProduct }) {
	...
	const monthlyRepayment = useMemo(() => {
		...
		const monthly =
			(loanAmount * rate * (1 + rate) ** n) / ((1 + rate) ** n - 1);
	...
	return (
		<div className="bg-white">
			<div className="space-y-8 mb-12">
				<div>
					<label className="text-[11px] font-black text-gray-300 uppercase tracking-widest mb-4 block">
						대출 신청 금액
					</label>
			...
			<div className="flex items-center justify-between p-6 bg-white border-2 border-rose-50 rounded-[32px] group cursor-pointer hover:border-rose-200 transition-colors">
				...
				<ArrowRight
					size={20}
					className="text-gray-200 group-hover:text-rose-600 transition-all"
				/>
			</div>
		</div>
	);
