	...
	const handleUpload = async () => {
		...
		try {
			...
			if (data.success) {
				...
				setErrorMsg(
					(data.message || "분석 중 오류가 발생했습니다.") + detailError,
				);
			}
		} catch (error) {
			console.error(error);
			setErrorMsg(
				"네트워크 또는 서버 에러가 발생했습니다. 파일 크기나 타임아웃을 확인하세요.",
			);
		} finally {
			setLoading(false);
		}
					...
					<div className="bg-[var(--treia-card)] border border-[var(--treia-card-border)] rounded-3xl p-8 flex flex-col gap-6 text-center shadow-xl">
						{preview ? (
							...
							<label className="border-2 border-dashed border-[var(--treia-card-border)] hover:border-amber-500/50 bg-[var(--treia-card)] rounded-2xl p-12 cursor-pointer transition-all flex flex-col items-center justify-center group overflow-hidden relative shadow-inner">
								...
								<UploadCloud
									size={48}
									className="text-gray-500 group-hover:text-amber-500 transition-colors mb-4"
								/>
					...
					{result ? (
						<div className="animate-in fade-in slide-in-from-bottom-4 duration-700 flex flex-col gap-6">
							...
							<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
								...
								<StatCard
									label="Win Rate"
									value={`${Math.round((result.winTrades / result.totalTrades) * 100)}%`}
									icon={<CheckCircle2 />}
										...
										<tbody className="text-sm font-medium">
											{result.trades.map((t, i) => (
												...
												>
													...
													<td className="p-4">
														<span
															className={`mr-2 px-1.5 py-0.5 rounded text-[10px] font-black tracking-widest ${t.type.toUpperCase() === "BUY" ? "bg-blue-500/20 text-blue-400" : "bg-red-500/20 text-red-400"}`}
														>
															{t.type}
														</span>
													...
													<td
														className={`p-4 text-right font-black font-mono ${t.profit > 0 ? "text-green-500" : "text-red-500"}`}
													>
														{t.profit > 0 ? "+" : ""}
														{t.profit.toFixed(2)}
													</td>
						...
						<div className="h-full bg-[var(--bg-card)] border border-[var(--border-color)] border-dashed rounded-3xl flex flex-col items-center justify-center p-12 text-center text-gray-600">
							<ScanIcon bg />
							<h3 className="text-xl font-bold text-gray-400 mb-2 mt-6">
								데이터 대기 중
							</h3>
							<p className="text-sm">
								좌측에서 스크린샷 이미지를 업로드하고 파싱을 진행해주세요.
							</p>
						</div>
					)}
				</div>
...
function StatCard({
	...
	return (
		<div className="bg-[var(--treia-card)] border border-[var(--treia-card-border)] rounded-2xl p-5 flex flex-col gap-3 shadow-md">
			...
			<span
				className={`text-2xl font-black font-outfit uppercase tracking-tighter ${color}`}
			>
				{value}
			</span>
		</div>
...
function ScanIcon({ bg = false }: { bg?: boolean }) {
	return (
		<div
			className={`w-12 h-12 flex items-center justify-center rounded-xl ${bg ? "bg-amber-500/10 text-amber-500/50" : "bg-transparent text-amber-500"}`}
		>
			<svg
				width="24"
				height="24"
