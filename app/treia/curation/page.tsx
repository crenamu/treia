  ...
  useEffect(() => {
    async function fetchAIInfo() {
      try {
        ...
        // 3. 필터링 및 상태 업데이트
        const finalArticles = summarizedResults.filter(
          (a): a is AIArticle => a !== null,
        );
        ...
        const expertData: AIArticle[] = [
          {
            ...
            summary_ko:
              "마틴게일을 배제한 순수 가격 액션 기반 골드 EA의 원리. 왜 상위 1% 트레이더들이 이 로직에 열광하는지 심층 분석합니다.",
            category: "자동매매",
            difficulty: "고급",
          ...
          },
          {
            ...
            summary_ko:
              "뉴욕 본장 시작 15분, 나스닥의 폭발적인 변동성을 수익으로 전환하는 알고리즘 설계도와 진입 필터 설정법을 공개합니다.",
            category: "매매기법",
            difficulty: "중급",
          ...
          },
          {
            ...
            summary_ko:
              "SMC(Smart Money Concepts)와 머신러닝을 결합하여 가상자산의 비정형 변동성을 예측하는 최신 기술 트렌드를 다룹니다.",
            category: "AI분석",
            difficulty: "고급",
            source: "MQL5 Expert",
          },
    ...
    <div className="container mx-auto px-4 py-12 flex flex-col gap-12">
      <header className="flex flex-col gap-4">
        ...
        <p className="text-gray-400 text-sm max-w-2xl leading-relaxed">
          글로벌 금융 시장의 화두를 AI가 실시간으로 포착하여 한국어로 정밀
          요약해 드립니다. <br />
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 p-4 rounded-xl bg-gray-900/50 border border-gray-800">
        <div className="flex items-center gap-3">
          <button className="px-4 py-2 rounded-lg bg-[#FFC107] text-black text-xs font-bold hover:shadow-[0_4px_12px_rgba(255,193,7,0.2)] transition-all">
            전체보기
          </button>
          <button className="px-4 py-2 rounded-lg bg-gray-800 text-gray-400 text-xs font-bold hover:bg-gray-700 transition-all border border-gray-700">
            자동매매 (EA)
          </button>
          <button className="px-4 py-2 rounded-lg bg-gray-800 text-gray-400 text-xs font-bold hover:bg-gray-700 transition-all border border-gray-700">
            매매기법
