...
const EA_DATA: EADevice[] = [
	{
		...
		isFree: false,
		description:
			"MQL5 프리미엄 등급. 마틴게일과 그리드를 철저히 배제하고 순수 가격 액션(Price Action)과 뉴스 필터로 골드의 변동성을 공략합니다.",
	...
	},
	{
		...
		isFree: false,
		description:
			"나스닥 시가 돌파(Opening Range Breakout) 전략의 정점. 5분/15분 본장 시작 직후의 강력한 모멘텀을 자동으로 포착합니다.",
	...
	},
	{
		...
		isFree: false,
		description:
			"강화학습(Reinforcement Learning) 기반의 비트코인 전용 EA. SMC(Smart Money Concepts) 유동성 분석 로직이 탑재되어 있습니다.",
		tags: ["BTC", "AI", "SMC"],
		stars: 4.9,
		author: "AlphaAlgo",
	},
	{
		...
		isFree: true,
		description:
			"RSI와 ATR 변동성을 결합하여 골드와 나스닥의 단기 타점을 화살표로 제공합니다. 수동 트레이더들의 진입 근거로 활용도가 높습니다.",
		...
		<div className="container mx-auto px-4 py-12 flex flex-col gap-12 max-w-7xl">
			<header className="flex flex-col gap-4">
				...
				<p className="text-gray-400 text-sm max-w-2xl leading-relaxed font-medium">
					엄격한 백테스트와 실거래 인증을 통과한 선별된 EA와 트레이딩 도구를
					만나보세요. <br />
			...
			{/* EA Builder Banner */}
			<div
				className="relative overflow-hidden rounded-[40px] bg-gradient-to-r from-amber-600 to-amber-400 p-10 md:p-14 flex flex-col md:flex-row items-center justify-between gap-8 group shadow-2xl shadow-amber-500/10 hover:shadow-amber-500/20 transition-all cursor-pointer"
				onClick={() => (window.location.href = "/ea/builder")}
			>
				...
				<div className="z-10 flex flex-col gap-4 text-black max-w-2xl">
					...
					<p className="text-sm md:text-base font-bold opacity-80 leading-relaxed">
						코딩을 몰라도 괜찮습니다. TREIA의 노코드 EA 제작 도구를 사용해
						나만의 트레이딩 전략(지표, 캔들패턴, 지지/저항 등)을 자동화된 봇으로
						만들어보세요.
					</p>
				</div>
				<div className="z-10">
					<Link
						href="/ea/builder"
						className="inline-flex items-center gap-2 px-8 py-4 bg-black text-white rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-gray-900 hover:scale-105 transition-all w-full md:w-auto justify-center"
					>
			...
			<div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
				<div className="lg:col-span-7 p-10 rounded-[40px] bg-gradient-to-br from-[#1E222D] to-[#12141A] border border-gray-800/50 flex flex-col gap-8 relative overflow-hidden group">
					...
					<p className="text-sm text-gray-400 leading-relaxed font-medium">
						수익률 조작으로 가득한 마켓에서, 트레이아는 실제 MyFxBook 실거래
						인증이 완료된 자료만을 게재합니다. 우리는 단순히 수익률만 보지
						않습니다. 낙폭(DD), 이익 보존율, 그리고 마틴게일 여부를 가려냅니다.
					</p>
				...
				<div className="lg:col-span-5 p-10 rounded-[40px] bg-[#14161B] border border-gray-800 flex flex-col gap-8">
					...
					<div className="flex flex-col gap-6">
						<StepItem
							num="01"
							title="MT5 터미널 설치"
							desc="메타트레이더 5 설치 및 계좌 연결"
						/>
						<StepItem
							num="02"
							title="파일 경로 설정"
							desc="MQL5/Experts 폴더 내 파일 복사"
						/>
						<StepItem
							num="03"
							title="AI 최적화 세팅"
							desc="트레이아 제공 셋파일 로드 및 활성화"
						/>
					</div>
	...
	return (
		<div className="group relative bg-[#181B21] border border-[#2D3340] rounded-[32px] p-6 overflow-hidden hover:border-amber-500/20 transition-all hover:-translate-y-1">
			...
			<div className="flex items-center justify-between mb-6">
				<span className="text-[9px] font-black text-gray-600 uppercase tracking-widest bg-gray-900/50 px-2 py-1 rounded">
					{ea.category}
				</span>
			...
			<div className="grid grid-cols-2 gap-4 border-y border-gray-800/50 py-5 mb-6">
				<div className="flex flex-col gap-1">
					...
					<span className="text-sm font-outfit font-black text-green-500 tracking-tighter">
						{ea.returns}
					</span>
				</div>
				<div className="flex flex-col gap-1">
					...
					<span className="text-sm font-outfit font-black text-red-500/80 tracking-tighter">
						{ea.drawdown}
					</span>
				</div>
			</div>
			...
			<div className="flex items-center justify-between">
				...
				<div
					className={`px-3 py-1.5 rounded-xl text-[10px] font-black tracking-widest uppercase ${ea.isFree ? "bg-green-500/10 text-green-500" : "bg-amber-500/10 text-amber-500"}`}
				>
