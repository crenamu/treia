		...
		<div className="container mx-auto px-4 py-12 flex flex-col gap-12 max-w-7xl">
			...
			<header className="flex flex-col gap-4">
				...
				<p className="text-gray-400 text-sm max-w-2xl leading-relaxed">
					CFD 트레이더들과 실시간으로 소통하고, 검증된 EA 전략과 수익 인증을
					통해 함께 성장하세요.
				</p>
			</header>
					...
					{loading ? (
						...
						<div className="relative p-12 lg:p-20 rounded-[40px] bg-gradient-to-b from-gray-900 to-black border border-gray-800 flex flex-col items-center text-center gap-8 overflow-hidden min-h-[400px] justify-center shadow-2xl">
							...
							<div className="flex flex-col gap-3">
								<h2 className="text-3xl font-outfit font-black text-white tracking-tight uppercase">
									Members Only
								</h2>
								<p className="text-gray-400 text-sm max-w-sm leading-relaxed">
									트레이아 커뮤니티의 실시간 전술과 수익 보고서는
									<br />
						...
						<div className="flex flex-col gap-4">
							{posts.length > 0 ? (
								posts.map((post) => (
									<article
										key={post.id}
										className="p-6 rounded-3xl bg-[var(--bg-card)] border border-[var(--border-color)] hover:border-orange-500/20 transition-all group relative"
									>
										<div className="flex items-center justify-between mb-4">
											<div className="flex items-center gap-3">
												<span className="px-2 py-0.5 rounded bg-gray-800 text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
													{post.category}
												</span>
												{post.isHot && (
											...
											<div className="flex items-center gap-4">
												{post.verificationStatus &&
													post.verificationStatus !== "none" && (
														<div
															className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest flex items-center gap-1.5 ${
														...
														{(!post.verificationStatus ||
															...
															<button
																onClick={(e) => {
																	...
																	if (
																		confirm(
																			"이 전략에 대해 운영진 검증을 요청하시겠습니까? (연구/검정 중으로 표시됩니다)",
																		)
																	) {
																		requestVerification(post.id).then(() =>
																			fetchCommunityPosts(),
																		);
																	}
																}}
										...
										{post.fileUrl && (
											...
											>
												...
												<div className="flex flex-col">
													<span className="text-xs font-bold text-gray-300 group-hover/file:text-white">
														{post.fileName}
													</span>
													<span className="text-[10px] text-gray-600 font-bold uppercase">
														전략 파일 다운로드
				...
				<aside className="lg:col-span-4 flex flex-col gap-8">
					<div className="p-8 rounded-3xl bg-gradient-to-br from-gray-900 to-black border border-gray-800 flex flex-col gap-6 relative overflow-hidden">
						...
						<div className="flex items-center gap-3">
							...
							<h2 className="text-xl font-outfit font-black text-white uppercase tracking-tight italic">
								수익률 랭킹 (2월)
							</h2>
						</div>
						<div className="flex flex-col gap-4">
				...
				<div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
					<div className="bg-[#12141a] border border-gray-800 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
						...
						<form
							...
							className="p-8 flex flex-col gap-6 max-h-[80vh] overflow-y-auto custom-scrollbar"
						>
...
function RankingItem({
	...
	return (
		<div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5 hover:border-amber-500/20 transition-all group">
			<div className="flex items-center gap-4">
				<span
					className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black ${rank === 1 ? "bg-amber-500 text-black" : "bg-gray-800 text-gray-500"}`}
				>
					{rank}
				</span>
				<div className="flex flex-col">
					<span className="text-sm font-bold text-gray-200 group-hover:text-amber-500 transition-colors">
						{name}
					</span>
					<span className="text-[10px] text-gray-600 font-bold">{prize}</span>
				</div>
			</div>
			<span className="text-sm font-outfit font-bold text-green-500">
				{returns}
			</span>
		</div>
	);
						...
						<div className="flex-1 flex flex-col gap-1">
							<div className="flex items-center gap-2">
								...
								<span className="text-[8px] text-gray-600 uppercase">
									{c.timestamp?.toDate
										? c.timestamp.toDate().toLocaleTimeString([], {
												hour: "2-digit",
												minute: "2-digit",
											})
										: "방금 전"}
