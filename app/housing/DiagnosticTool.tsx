...
export default function HousingDiagnosticTool({
	...
	const [result, setResult] = useState<null | "eligible" | "not-eligible">(
		null,
	);
	...
	if (result) {
		return (
			<div className="bg-white rounded-[40px] p-10 md:p-12 text-center animate-in fade-in zoom-in duration-500">
				<div
					className={`w-24 h-24 mx-auto rounded-full flex items-center justify-center mb-8 shadow-xl ${
						result === "eligible"
				...
				<div className="grid grid-cols-1 gap-3 mb-10">
					<ScoreItem
						label="소득 적합도"
						score={totalScore > 40 ? "매우 높음" : "보통"}
						active
					/>
					<ScoreItem
						label="자산 유지도"
						score={totalScore > 50 ? "안전" : "주의"}
						active={totalScore > 50}
					/>
				</div>
		...
		<div className="bg-white rounded-[40px] p-8 md:p-12 relative overflow-hidden">
			...
			<div className="mt-12 pt-8 border-t border-gray-50 flex items-center gap-3 text-gray-400">
				...
				<p className="text-[10px] font-medium leading-relaxed">
					입력하신 정보는 진단 후 즉시 파기되며, 핀테이블의 AI가 익명으로 분석을
					진행합니다.
				</p>
			</div>
		</div>
...
function ScoreItem({
	...
	return (
		<div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl">
			...
			<span
				className={`text-sm font-black ${active ? "text-blue-600" : "text-red-500"}`}
			>
				{score}
			</span>
		</div>
