...
export default function HousingPage() {
	...
	const filtered = notices.filter(
		(n) => region === "전체" || n.location.includes(region),
	);
				...
				<div className="container mx-auto max-w-5xl px-6">
					...
					<div className="flex flex-col md:flex-row gap-4">
						<div className="flex-1 relative group">
							<Search
								className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-blue-600 transition-colors"
								size={20}
							/>
							<input
								type="text"
						...
						<div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
							<div className="text-center md:text-left">
								...
								<p className="text-blue-100 font-medium text-lg leading-relaxed max-w-lg">
									LH/SH 청약은 점수 싸움입니다. 3분 만에 끝나는 AI 진단으로 내가
									당첨될 수 있는 공고와 최적의 브릿지 금융 상품을 매칭받으세요.
								</p>
							</div>
							<button
					...
					<div className="flex flex-col md:flex-row items-center gap-20">
						<div className="flex-1 space-y-8">
							...
							<p className="text-gray-400 font-medium text-lg leading-relaxed">
								핀테이블은 매일 2,400개 이상의 공공데이터를 수집합니다. 단순
								공고 전달을 넘어 당첨 가점, 주변 시세, 인프라 평점까지 제공하는
								국내 유일의 리얼데이터 큐레이션 플랫폼입니다.
							</p>
						...
						<div className="w-full max-w-sm">
							<div className="bg-white/5 backdrop-blur-md rounded-[40px] p-10 border border-white/10 space-y-8">
								...
								<input
									...
									className="w-full bg-white/10 border border-white/10 rounded-2xl p-4 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-gray-600"
								/>
								<button className="w-full py-5 bg-blue-600 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-[1.02] transition-all">
									실시간 리포트 구독
								</button>
								<p className="text-center text-[10px] text-gray-500 font-bold opacity-50 uppercase tracking-widest">
									FinTable Intelligence Team
								</p>
							</div>
						</div>
					</div>
		...
		>
			<div className="flex items-center justify-between mb-8">
				<div className="flex items-center gap-2">
					<span
						className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest text-white ${notice.status === "접수중" ? "bg-blue-600" : "bg-gray-400"}`}
					>
						{notice.status}
					</span>
					<span className="px-3 py-1.5 rounded-xl bg-gray-50 text-gray-400 text-[10px] font-black uppercase tracking-widest">
						{notice.provider}
					</span>
				</div>
				<ShareSaveButtons
					id={`housing_${notice.id}`}
					title={notice.title}
					type="housing"
				/>
			</div>
			...
			<div className="grid grid-cols-2 gap-6 pt-8 border-t border-gray-100">
				<div className="flex flex-col">
					...
					<p className="text-base font-black text-gray-900">
						{notice.date.split("~")[1]?.trim() || "상시 모집"}
					</p>
				</div>
				{diagnosed ? (
					<div className="text-right flex flex-col items-end">
						<span className="text-[10px] text-blue-600 font-black uppercase tracking-widest mb-2 flex items-center gap-1">
							<Sparkles size={12} /> Prob
						</span>
					...
					<div className="text-right flex flex-col items-end">
						<span className="text-[10px] text-gray-300 font-black uppercase tracking-widest mb-2">
							Prob
						</span>
...
function InfoRow({
	...
	return (
		<div className="flex items-center gap-5">
			<div
				className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${isPoint ? "bg-blue-50 text-blue-600" : "bg-gray-50 text-gray-400 group-hover:bg-white group-hover:shadow-lg border border-transparent"}`}
			>
				{icon}
			</div>
			<div className="flex-1 min-w-0">
				<p className="text-[10px] font-black text-gray-300 uppercase tracking-widest mb-1 leading-none">
					{label}
				</p>
				<p
					className={`text-base font-bold truncate ${isPoint ? "text-blue-600" : "text-gray-900 group-hover:text-blue-600 transition-colors"}`}
				>
					{value}
				</p>
			</div>
...
function NoticeSkeleton() {
	return (
		<div className="bg-gray-100 rounded-[40px] h-[450px] animate-pulse"></div>
	);
}
