...
import { subscribePageAnalysis, PageAnalysis } from "@/lib/analysis-service";
...
const defaultScenarios = [
  { title: "Price Discovery Phase", desc: "역사적 신고가 $5,300 돌파 시도는 단순한 상승을 넘어 새로운 가치 영역을 탐색하는 프로세스입니다.", color: "bg-cyan-400" },
  { title: "Institutional Flush", desc: "주요 POC 이탈 시 하단 유동성 거점($5,130)까지의 급격한 가격 이동(Flash)이 빈번하게 관찰됩니다.", color: "bg-red-500" },
];

const defaultAnalysisText = "목요일 저점($5,130)에서의 강력한 V자 반등은 단순한 상승이 아닌 기관의 대량 매집 신호로 해석됩니다. 현재 $5,278 부근의 주간 POC 안착 여부가 다음 세션의 방향성을 결정짓는 핵심 관찰 포인트입니다.";
      ...
      <div className="flex flex-col gap-8 w-full">
         ...
         <div className="p-5 md:p-6 rounded-3xl bg-blue-500/5 border border-blue-500/10 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 mt-4">
            <div className="flex items-center gap-2 shrink-0">
               ...
               <h4 className="font-bold text-[11px] md:text-xs text-blue-400 uppercase tracking-widest">분석 가이드</h4>
            </div>
            <p className="text-[11px] md:text-xs text-gray-400 leading-relaxed font-medium">
               모든 데이터는 실시간 브로커 서버와 99.9% 동기화됩니다. 차트상의 레벨은 교육적 관점 공유를 일목요연하게 파악하기 위한 참고 자료입니다.
            </p>
         </div>
      </div>
