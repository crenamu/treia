...
export default function SavingsPage() {
	...
	const bestMax =
		products.length > 0
			? [...products].sort(
					(a, b) =>
						(b.bestOption?.intr_rate2 || 0) - (a.bestOption?.intr_rate2 || 0),
				)[0]
			: null;
			...
			<main className="container mx-auto max-w-5xl px-6 pt-12 md:pt-20">
				...
				>
					...
					<h1 className="text-5xl md:text-7xl font-outfit font-black text-gray-900 leading-[0.95] mb-8 tracking-tighter">
						목돈 마련의
						<br />
						<span className="text-purple-600 underline underline-offset-8 decoration-8 decoration-purple-100">
							가장 빠른 길
						</span>
					</h1>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-12">
					<div className="bg-purple-600 rounded-[32px] p-8 text-white flex items-center justify-between shadow-xl shadow-purple-500/20 group overflow-hidden relative">
						<div className="relative z-10">
							<p className="text-[10px] font-black uppercase tracking-[3px] text-purple-200 mb-2">
								현시점 최고 금리
							</p>
							<h2 className="text-5xl font-outfit font-black mb-1">
								연 {bestMax?.bestOption?.intr_rate2.toFixed(2)}%
							</h2>
						...
						<Rocket
							size={80}
							className="text-purple-500/30 absolute right-[-10px] bottom-[-10px] rotate-12 group-hover:scale-110 transition-transform"
						/>
					</div>
					<div className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm flex flex-col justify-center">
						...
						<Link
							href="/calculator"
							className="w-full py-5 bg-gray-50 hover:bg-gray-100 rounded-2xl flex items-center justify-center gap-3 text-sm font-black transition-all"
						>
							<Wallet size={18} className="text-purple-600" /> 목표 금액별 월
							납입액 계산
						</Link>
					</div>
				</div>
						...
						<div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
							...
							<div className="flex items-center gap-4">
								<p className="text-xs font-bold text-gray-400">
									총 <span className="text-gray-900">{products.length}개</span>{" "}
									상품
								</p>
											...
											>
												<div className="flex items-center gap-8 flex-1 w-full">
													...
													<div className="flex-1 min-w-0">
														<p className="text-[11px] font-black text-gray-400 uppercase tracking-tight mb-1">
															{p.kor_co_nm}
														</p>
														<h3 className="text-xl font-black text-gray-900 group-hover:text-purple-700 transition-colors truncate mb-2">
															{p.fin_prdt_nm}
														</h3>
														<div className="flex items-center gap-3">
															<span className="text-xs font-bold text-gray-400">
																{p.bestOption?.save_trm}개월
															</span>
															<div className="w-1 h-1 rounded-full bg-gray-200"></div>
															<span className="text-xs font-bold text-gray-400">
																{p.bestOption?.intr_rate_type_nm}
															</span>
														</div>
													</div>
												</div>
												...
												<div className="flex items-center gap-12 mt-8 md:mt-0 w-full md:w-auto justify-between md:justify-end">
													<div className="text-right">
														...
														<div className="flex items-baseline gap-1">
															<span className="text-4xl font-outfit font-black text-purple-600 leading-none">
