...
export async function GET() {
	try {
		...
		if (!isServer)
			return NextResponse.json({ success: false, msg: "Server only" });
		...
		if (!fs.existsSync(jsonPath)) {
			return NextResponse.json({
				success: false,
				message: "ea_history_parsed.json 파일을 찾을 수 없습니다.",
			});
		}
		...
		const typeStr =
			String(latestTrade.type).toLowerCase() === "buy" ? "BUY" : "SELL";
