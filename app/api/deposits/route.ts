...
// Mock data as fallback for "Premium" experience even when gov API fails
const MOCK_PRODUCTS = [
  {
    fin_prdt_cd: 'MOCK_001',
    kor_co_nm: '핀테이블은행',
    fin_prdt_nm: 'FinTable 프리미엄 정기예금',
    join_way: '인터넷,스마트폰',
    spcl_cnd: '첫 거래 고객 0.5%p 우대\n마케팅 동의 시 0.1%p 우대',
    mtrt_int: '만기 시 일시 지급',
    etc_note: '해당 데이터는 API 장애 시 제공되는 시뮬레이션용 데이터입니다.',
    options: [
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (data.result?.err_cd && data.result.err_cd !== '000') {
      ...
      // Fallback to mock if API returns error (like monthly quota exceeded)
      return NextResponse.json({ products: formatProducts(MOCK_PRODUCTS, trm), total: MOCK_PRODUCTS.length, isMock: true });
    }
