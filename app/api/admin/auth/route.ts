...
const SECRET_KEY = new TextEncoder().encode(
	process.env.ADMIN_SESSION_SECRET || "treia_default_secret_key_2026",
);
	...
	try {
		...
		if (!process.env.ADMIN_PASSWORD || !process.env.ADMIN_OTP_SECRET) {
			return NextResponse.json(
				{ success: false, message: "ENV 설정 누락" },
				{ status: 500 },
			);
		}
		...
		return NextResponse.json(
			{ success: false, message: `서버 오류: ${message}` },
			{ status: 500 },
		);
	}
}
