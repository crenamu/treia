'use client'
import { Monitor, Download, Zap, ShieldCheck, Play, ArrowRight, Settings, Cpu } from "lucide-react";
    ...
    <div className="container mx-auto px-4 py-12 flex flex-col gap-16 max-w-5xl">
      ...
      <header className="flex flex-col items-center text-center gap-6">
        ...
        <h1 className="text-4xl md:text-5xl font-outfit font-black text-white tracking-tight uppercase leading-[1.1]">
          MT5 자동매매 설치 <br /> <span className="text-amber-500">완벽 마스터 가이드</span>
        </h1>
      ...
      <section className="p-8 md:p-12 rounded-[40px] bg-gradient-to-br from-gray-900 to-black border border-gray-800 flex flex-col gap-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
           ...
           <h2 className="text-2xl font-outfit font-black text-white uppercase italic tracking-tight">트레이아 PRO의 연구/검증 원칙</h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div className="flex flex-col gap-3">
              <h4 className="text-sm font-bold text-white flex items-center gap-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-amber-500" /> 커뮤니티 전략 검증 (Audit)
              </h4>
              <p className="text-xs text-gray-500 leading-relaxed font-medium">
                 커뮤니티에 올라온 아티클은 AI와 운영진이 리스크 요인(마틴게일, 그리드 여부)을 직접 연구합니다. 
                 검증 배지가 달린 전략만을 신뢰하세요.
              </p>
           </div>
           <div className="flex flex-col gap-3">
              <h4 className="text-sm font-bold text-white flex items-center gap-2">
                 <div className="w-1.5 h-1.5 rounded-full bg-amber-500" /> 백테스팅 & 포워드 테스트
              </h4>
              <p className="text-xs text-gray-500 leading-relaxed font-medium">
                 모든 추천 EA는 최소 3년 이상의 틱데이터 백테스팅과 6개월 이상의 실거래 포워드 데이터 분석을 거칩니다.
              </p>
           </div>
        </div>
      </section>
      ...
      <div className="flex flex-col items-center gap-6 py-12 border-t border-gray-800">
         <h3 className="text-xl font-bold text-white text-center">지금 바로 첫 번째 자동매매를 시작해볼까요?</h3>
...
function GuideStep({ num, title, desc, icon, isHighlight }: { num: string; title: string; desc: string; icon: React.ReactNode; isHighlight?: boolean }) {
  return (
    <div className={`p-8 rounded-3xl border transition-all flex flex-col md:flex-row gap-6 md:items-center ${isHighlight ? 'bg-amber-500/5 border-amber-500/30' : 'bg-gray-900/40 border-gray-800 hover:border-gray-700'}`}>
       <div className="flex items-center gap-4 flex-1">
          <span className="text-3xl font-outfit font-black text-gray-800">{num}</span>
          ...
          <div className="flex flex-col gap-1">
             ...
             <p className="text-xs text-gray-500 leading-relaxed font-medium max-w-lg">{desc}</p>
          </div>
       </div>
