...
export default function DepositDetailPage() {
  ...
  useEffect(() => {
    // API endpoint for single product not implemented yet, so we fetch all and find the one
    // In real app, /api/deposits/[id] would be better
    fetch(`/api/deposits`)
      .then(r => r.json())
      .then(data => {
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm">
              ...
              <div className="space-y-4">
                <p className="text-xs text-gray-400 font-medium italic">유사한 금리의 추천 상품이 준비 중입니다.</p>
              </div>
            </div>
          </div>
        </div>
