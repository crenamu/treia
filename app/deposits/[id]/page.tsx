			...
			details={[
				{
					...
					value: product.max_limit
						? `최소 1만원 ~ 최대 ${Math.floor(product.max_limit / 10000)}만원`
						: "최소 1만원 이상",
				},
...
function DepositRateSimulator({ product }: { product: DepositProduct }) {
	...
	const beforeTaxInterest = Math.floor(
		amount * (simulatedRate / 100) * (months / 12),
	);
					...
					{product.options
						...
						.map((opt, i) => (
							...
							>
								...
								<span
									className={`text-[10px] font-medium ${trmIndex === i ? "text-gray-300" : "text-gray-400"}`}
								>
									기본 {opt.intr_rate}%
								</span>
							</button>
				...
				{primeConditions.slice(0, 6).map((cond, i) => (
					...
					>
						<div className="flex-1 pr-6">
							<p
								className={`text-sm md:text-base font-bold transition-colors ${checkedItems[i] ? "text-blue-900" : "text-gray-600"}`}
							>
						...
						<div
							className={`w-14 h-7 rounded-full transition-all relative shrink-0 ${checkedItems[i] ? "bg-blue-600" : "bg-gray-100"}`}
						>
							<div
