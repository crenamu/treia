...
export default function CardDetailPage() {
	...
	const externalLink = useMemo(() => {
		...
		const bankKey = Object.keys(BANK_URLS).find((key) =>
			product.company.includes(key),
		);
	...
	return (
		<PremiumProductTemplate
			...
			metrics={[
				{
					label: "카드 종류",
					value: product.type === "credit" ? "신용카드" : "체크카드",
					icon: <CreditCard size={14} />,
				},
				{
					label: "전월 실적",
					value: product.prevMonthRecord,
			...
			details={[
				...
				{ label: "대표 혜택", value: product.bestBenefit },
				{
					label: "상세 안내",
					value:
						"전 가맹점 결제 시 포인트 적립 및 업종별 맞춤 할인 혜택을 제공하며, 카드사 이벤트를 통해 추가 혜택을 받으실 수 있습니다. 자세한 내용은 상품 공시실 및 카드사 홈페이지를 참조하세요.",
					isText: true,
				},
			]}
			externalLink={
				product.externalLink ||
				getSmartLandingUrl(product.company, product.name)
			}
			ranking={{
			...
			onAction={() =>
				window.open(product.externalLink || externalLink, "_blank")
			}
		/>
	);
}
		...
		<div className="bg-white">
			...
			<div className="bg-indigo-50/50 rounded-[40px] p-10 text-center border border-indigo-100 mb-8">
				...
				<h3 className="text-4xl md:text-5xl font-black text-indigo-900 tracking-tighter">
					약 {estimatedSaving.toLocaleString()}
			...
			<div className="grid grid-cols-1 gap-4">
				{product.benefits.map((b, i) => (
					<div
						key={i}
						className="flex items-center gap-4 p-5 bg-white border border-gray-50 rounded-3xl"
					>
