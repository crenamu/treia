...
export default function CalculatorPage() {
	...
	const results = useMemo(() => {
		...
		if (type === "deposit") {
			if (isCompound) {
				totalInterest = amount * ((1 + monthlyRate) ** trm - 1);
			} else {
			...
			if (isCompound) {
				totalInterest =
					amount *
						(((1 + monthlyRate) ** trm - 1) / monthlyRate) *
						(1 + monthlyRate) -
					amount * trm;
			} else {
						...
						<div className="bg-gray-900 rounded-[40px] p-8 text-white relative overflow-hidden group">
							...
							<div className="relative z-10">
								...
								<p className="text-sm text-gray-400 font-medium leading-relaxed mb-6">
									현재 설정하신 {rate}% 금리는 시중 은행 상위 5% 수준입니다.
									비과세 혜택 적용 시 실질 수익률이 약 0.7%p 상승합니다.
								</p>
								<Link
									href="/"
									className="text-[10px] font-black uppercase tracking-widest text-green-500 flex items-center gap-2 group-hover:translate-x-2 transition-transform"
								>
					...
					<div className="lg:col-span-7 space-y-6">
						...
						<div className="bg-white rounded-[60px] p-10 md:p-14 shadow-2xl shadow-gray-200/50 border border-gray-100 flex flex-col md:flex-row gap-12 items-center">
							<div className="flex-1 space-y-8 w-full">
								<div>
									<p className="text-[10px] font-black text-gray-400 uppercase tracking-[3px] mb-4">
										만기 시 총 수령액
									</p>
								...
								<div className="grid grid-cols-2 gap-8 pt-8 border-t border-gray-100">
									...
									<ResultItem
										label="실효 수익률"
										value={`${(((results.netInterest / results.principal) * 100) / (trm / 12)).toFixed(2)}%`}
										raw
									/>
								</div>
							</div>
							...
							<div className="w-full md:w-48 flex flex-col items-center gap-6">
								<div className="relative w-40 h-40">
										...
										<motion.circle
											...
											animate={{
												strokeDashoffset:
													100 -
													(results.netInterest /
														(results.principal + results.netInterest)) *
														400,
											}}
									...
									<div className="absolute inset-0 flex flex-col items-center justify-center">
										...
										<p className="text-xl font-outfit font-black text-green-600">
											{((results.netInterest / results.total) * 100).toFixed(1)}
											%
										</p>
									</div>
								</div>
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm flex flex-col">
								<h6 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-8">
									Asset Growth Flow
								</h6>
								<div className="flex-1 flex items-end gap-2 min-h-[120px]">
									{results.monthlyData.map((d, i) => (
										<motion.div
											...
											initial={{ height: 0 }}
											animate={{
												height: `${(d.value / results.total) * 100}%`,
											}}
		...
		<div className="space-y-4">
			...
			<div className="relative h-1 bg-gray-100 rounded-full">
				<motion.div
					...
					style={{
						width: `${Math.min((value / (label.includes("금리") ? 10 : 100000000)) * 100, 100)}%`,
					}}
				/>
			</div>
			{marks && (
				<div className="flex justify-between">
					{marks.map((m) => (
						<button
							key={m}
							className="text-[9px] font-bold text-gray-300 hover:text-green-600 transition-colors"
							onClick={() =>
								onChange(
									Number(m.replace(/[^0-9]/g, "")) *
										(m.includes("만") ? 10000 : 1),
								)
							}
						>
							{m}
...
function ToggleRow({
	...
	return (
		...
		>
			...
			<div
				className={`w-12 h-6 rounded-full p-1 transition-all flex items-center ${active ? "bg-green-600" : "bg-gray-200"}`}
			>
				<div
					className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${active ? "translate-x-6" : "translate-x-0"}`}
				/>
			</div>
		</div>
	);
...
function ResultItem({
	...
	return (
		<div className="space-y-1">
			...
			<p
				className={`text-lg font-outfit font-black ${highlight ? "text-green-600" : isNegative ? "text-pink-500" : "text-gray-900"}`}
			>
				{raw ? value : `${Number(value).toLocaleString()}원`}
			</p>
		</div>
	);
}
...
function RecommendedLink({ title, rate }: { title: string; rate: string }) {
	return (
		<Link
			href="/"
			className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl group border border-transparent hover:border-green-100 hover:bg-white transition-all shadow-sm"
		>
			...
			<div className="text-right shrink-0">
				...
				<p className="text-[9px] font-bold text-gray-300 flex items-center justify-end">
