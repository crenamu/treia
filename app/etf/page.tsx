...
import { ArrowUpRight, BarChart3, Globe, Info, Star, Zap } from "lucide-react";
			...
			<main className="container mx-auto max-w-5xl px-6 py-12 md:py-20">
				...
				<div className="flex flex-col items-center text-center space-y-6 mb-20">
					...
					<p className="text-gray-500 font-medium max-w-xl leading-relaxed">
						어떤 종목을 살지 고민된다면? 시장 전체에 투자하는 ETF가 정답입니다.
						<br />
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-20">
					{ETF_CATEGORIES.map((cat, idx) => (
						<div
							key={cat.title}
							className="group bg-white rounded-[40px] p-8 md:p-10 border border-gray-100 shadow-sm hover:border-emerald-200 hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500"
						>
							<div className="flex items-start justify-between mb-8">
								<div
									className={`w-16 h-16 ${cat.color} rounded-2xl flex items-center justify-center transition-transform group-hover:scale-110`}
								>
									{cat.icon}
								</div>
								<div className="flex flex-wrap gap-2 justify-end">
									{cat.tags.map((tag) => (
										<span
											key={tag}
											className="px-3 py-1 bg-gray-50 text-[10px] font-black text-gray-400 rounded-lg group-hover:bg-emerald-50 group-hover:text-emerald-600 transition-colors"
										>
											{tag}
										</span>
									))}
							...
							<button
								type="button"
								className="flex items-center gap-2 text-sm font-black uppercase tracking-widest text-emerald-600 group-hover:gap-4 transition-all"
							>
				...
				<div className="bg-emerald-600 rounded-[48px] p-8 md:p-16 text-white overflow-hidden relative">
					<div className="relative z-10 max-w-2xl">
						...
						<p className="text-emerald-50/70 font-medium text-lg leading-relaxed mb-12">
							금(Gold)과 외환 시장의 변동성을 활용한 Treia의 AI 자동매매
							시스템으로 스마트하게 대응하세요.
						</p>
						<Link
							href="/treia"
				...
				<div className="mt-12 p-8 bg-gray-50 rounded-[32px] border border-gray-100 flex items-start gap-4">
					...
					<div className="space-y-2">
						...
						<p className="text-sm text-gray-500 font-medium leading-relaxed">
							ETF는 실적 배당형 상품으로 예금자보호법에 따라 예금보험공사가
							보호하지 않습니다. 운용 결과에 따라 원금 손실이 발생할 수 있으니
							투자 전 설명서를 반드시 확인하시기 바랍니다.
						</p>
					</div>
				</div>
			</main>
