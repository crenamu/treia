...
export default function SavedPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setBookmarks(docs.sort((a, b) => (b.timestamp?.seconds || 0) - (a.timestamp?.seconds || 0)))
      setLoading(false)
    }, (error) => {
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[var(--bg-beige)] flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-3xl font-black text-gray-900 mb-6 tracking-tighter uppercase">Security Check Required</h1>
        <p className="text-gray-500 font-medium max-w-sm mb-12 leading-relaxed text-lg">
          당신의 개인 핀테크 대시보드를 확인하려면<br/>안전한 로그인이 필요합니다.
        </p>
      ...
      <main className="container mx-auto max-w-6xl px-6 pt-12 md:pt-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-16">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-2xl shadow-sm border border-gray-100">
              ...
              <span className="text-[10px] font-black uppercase tracking-[2px] text-gray-400">My Financial Folder</span>
            </div>
            ...
            <p className="text-gray-500 font-medium text-lg leading-relaxed">
               {user.displayName || '사용자'}님, 엄선하신 {bookmarks.length}개의 기회들이 이곳에 모였습니다.
            </p>
          </motion.div>
             ...
             {['전체', '금융', '주거'].map(tab => (
               <button
                 ...
                 className={`px-8 py-4 rounded-[22px] text-xs font-black transition-all tracking-[1px] uppercase ${
                   activeTab === tab ? 'bg-gray-900 text-white shadow-lg' : 'text-gray-400 hover:text-gray-600'
                 }`}
               >
                 {tab}
               </button>
        ...
        <AnimatePresence mode="wait">
          {filteredBookmarks.length === 0 ? (
            ...
            >
              ...
              <h3 className="text-2xl font-black text-gray-900 mb-4 tracking-tight">수집된 폴더가 비어있습니다</h3>
              <p className="text-gray-400 font-medium mb-12 text-lg">당신에게 필요한 금융과 주거의 기회를 직접 채워보세요.</p>
              <Link 
                href="/"
              ...
              {filteredBookmarks.map((item, idx) => (
                ...
                >
                   <div className="bg-white rounded-[39px] p-6 flex flex-col md:flex-row md:items-center gap-8">
                     <div className={`w-16 h-16 rounded-[28px] flex items-center justify-center shrink-0 shadow-lg group-hover:scale-110 transition-transform ${
                       item.type === 'housing' ? 'bg-blue-600 text-white shadow-blue-500/20' : 'bg-green-500 text-white shadow-green-500/20'
                     }`}>
                       {item.type === 'housing' ? <HomeIcon size={28} /> : <Landmark size={28} />}
                     </div>

                     <div className="flex-1 min-w-0">
                       <div className="flex items-center gap-3 mb-2">
                         <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest text-white ${item.type === 'housing' ? 'bg-blue-400' : 'bg-green-400'}`}>
                           {item.type === 'housing' ? 'HOUSING' : 'FINANCE'}
                         </span>
                         <span className="text-[10px] text-gray-300 font-bold flex items-center gap-1">
                            <Calendar size={12} /> {item.timestamp ? new Date(item.timestamp.seconds * 1000).toLocaleDateString() : '최근'}
                         </span>
                       </div>
        ...
        <div className="mt-24 space-y-12">
            <div className="flex items-center justify-between px-2">
               ...
               <Link href="/" className="text-xs font-bold text-gray-400 hover:text-gray-900">모두 보기 <ChevronRight size={14} className="inline" /></Link>
            </div>
...
function InsightCard({ label, value, sub, icon, color }: { label: string, value: string, sub: string, icon: React.ReactNode, color: string }) {
   return (
      <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-xl shadow-gray-200/10 flex items-center gap-6 group hover:-translate-y-1 transition-all duration-500">
         <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${color === 'green' ? 'bg-green-50 text-green-600' : color === 'blue' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}>
            {icon}
         </div>
         <div className="min-w-0">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{label}</p>
            <p className="text-2xl font-outfit font-black text-gray-900 leading-none mb-1.5">{value}</p>
...
function RecommendCard({ title, desc, icon, color }: { title: string, desc: string, icon: React.ReactNode, color: 'blue' | 'green' }) {
   return (
      <div className="bg-white rounded-[48px] p-10 border border-gray-100 shadow-sm flex flex-col md:flex-row items-center gap-10 hover:shadow-2xl hover:border-blue-100 transition-all group overflow-hidden relative">
         <div className={`absolute top-0 right-0 w-32 h-32 opacity-5 blur-3xl ${color === 'blue' ? 'bg-blue-600' : 'bg-green-600'}`}></div>
         <div className={`w-16 h-16 rounded-[28px] flex items-center justify-center shrink-0 transition-all duration-500 ${color === 'blue' ? 'bg-blue-50 text-blue-600 group-hover:bg-blue-600 group-hover:text-white' : 'bg-green-50 text-green-600 group-hover:bg-green-600 group-hover:text-white'}`}>
            {icon}
         </div>
