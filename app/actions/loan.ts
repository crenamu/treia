...
export async function getLoans(
  ...
  try {
    ...
    let products = Object.values(mergedProductMap)
      ...
      .map(p => {
        const sortedOpts = [...p.options].sort((a, b) => (a.lend_rate_min || 99) - (b.lend_rate_min || 99));
...
function parseLoanTags(name: string, joinWay: string, type: string): string[] {
  ...
  if (text.includes('비대면') || text.includes('인터넷') || text.includes('스마트폰')) tags.push('방문없이가입');
...
function generateMockLoans(type: 'mortgage' | 'rent' | 'credit'): LoanProduct[] {
