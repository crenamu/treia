...
## 7. 오늘의 작업 목표 (2026.03.04)

- **매매일지 데이터베이스(DB) 영구 저장**: AI로 분석한 매매 데이터를 Firestore `treia_journals` 컬렉션에 사용자별로 저장.
- **통계 대시보드 구축**: 누적된 매매일지 데이터를 바탕으로 총 수익, 승률, 평균 보유 시간 등의 통계를 시각화.
- **인증 시스템 연동**: 사용자별 독립된 매매 데이터를 관리하기 위해 Firebase Auth 연동 강화.
