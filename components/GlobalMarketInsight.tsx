import { TrendingUp, Globe, Coins, Wallet } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col md:flex-row items-center gap-12 p-8 md:p-12 bg-white rounded-[56px] border border-gray-100 shadow-sm relative overflow-hidden group">
      ...
      <div className="flex items-center gap-6 shrink-0">
        ...
        <div>
          <h4 className="text-xl font-black text-gray-900 tracking-tight">Global Insights</h4>
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Real-time Market Data</p>
        </div>
      </div>
        ...
        {metrics.map((m) => (
          <div key={m.label} className="flex flex-col gap-2">
            ...
            <div className="flex items-end gap-3">
              <span className="text-2xl font-black text-gray-900 tracking-tighter">{m.value}</span>
              <div className={`flex items-center gap-1 text-[11px] font-black ${m.up ? 'text-green-500' : 'text-red-500'}`}>
      ...
      <div className="hidden lg:flex items-center gap-3 px-6 py-4 bg-gray-50 rounded-2xl border border-gray-100">
         ...
         <span className="text-xs font-bold text-gray-500 italic">Market volatility remains moderate</span>
      </div>
    </div>
  )
}
