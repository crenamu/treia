...
export default function EconomicCalendar() {
	...
	const [filter, setFilter] = useState<"today" | "tomorrow" | "thisweek">(
		"today",
	);
		...
		<div className="bg-[#0F1115] border border-[#2B303B] rounded-3xl p-6 flex flex-col gap-4 relative overflow-hidden h-[300px]">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2 px-2">
				<div className="flex items-center gap-2">
					...
					<h4 className="font-bold text-xs uppercase tracking-widest text-[#FFC107]">
						핵심 경제지표 (★★★★★)
					</h4>
				</div>
					...
					<div className="flex flex-col gap-2">
						{filteredData.map((item, idx) => {
							...
							return (
								<div
									key={idx}
									className={`p-4 rounded-xl border ${isToday ? "border-amber-500/50 bg-amber-500/5" : "border-gray-800/80 bg-black/20"} flex flex-col gap-3 transition-colors hover:border-amber-500/30`}
								>
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-2">
											<span
												className={`text-[10px] font-black tracking-widest px-2 py-0.5 rounded-sm uppercase ${
													item.country === "USD"
											...
											<span
												className={`text-xs font-bold flex items-center gap-1 ${isToday ? "text-amber-400" : "text-gray-400"}`}
											>
										...
										{isToday && (
											<span className="text-[9px] font-black text-amber-500 animate-pulse border border-amber-500/30 px-1.5 py-0.5 rounded uppercase">
												Today
											</span>
										)}
									</div>
									...
									<div className="grid grid-cols-3 gap-2 mt-1 pt-3 border-t border-gray-800/50">
										<div className="flex flex-col text-[10px]">
											...
											<span
												className={`font-mono font-bold ${item.actual !== "-" && item.actual !== "" ? "text-cyan-400" : "text-gray-600"}`}
											>
												{item.actual || "-"}
											</span>
										</div>
