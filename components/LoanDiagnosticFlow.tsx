...
const STEPS: Step[] = [
	{
		id: 1,
		title: "약관 동의",
		subtitle: "대출을 찾아오기 위해 꼭 필요한 동의만 받아요",
		icon: <ShieldCheck className="text-emerald-500" />,
	},
	{
		id: 2,
		title: "직업 선택",
		subtitle: "정확한 금리 비교를 위해 지금 하고 계신 일을 알려주세요",
		icon: <User className="text-blue-500" />,
	},
	{
		id: 3,
		title: "분석 시작",
		subtitle: "금융기관 120여 곳에서 당신을 위한 최고의 조건을 찾는 중입니다",
	},
];
...
export default function LoanDiagnosticFlow({
	onClose,
	onComplete,
}: {
	onClose: () => void;
								...
								<div className="space-y-6">
									<div className="space-y-4">
										{TERMS.map((term, i) => (
											...
											>
												<span
													className={`text-base font-bold transition-colors ${checkedTerms[i] ? "text-gray-900" : "text-gray-300 group-hover:text-gray-500"}`}
												>
													{term}
												</span>
												<CheckCircle2
													size={24}
													className={`transition-colors ${checkedTerms[i] ? "text-emerald-500" : "text-gray-100"}`}
												/>
											</div>
										))}
									</div>
							...
							{stepIndex === 1 && (
								<div className="grid grid-cols-1 gap-3">
									{JOBS.map((job) => (
										...
										>
											<span
												className={`text-lg font-black ${selectedJob === job.id ? "text-white" : "text-gray-900"}`}
											>
												{job.label}
											</span>
			...
			{!isAnalyzing && (
				<footer className="p-8 bg-white border-t border-gray-50">
					<button
						onClick={handleNext}
						disabled={
							(stepIndex === 0 && !checkedTerms.every(Boolean)) ||
							(stepIndex === 1 && !selectedJob)
						}
