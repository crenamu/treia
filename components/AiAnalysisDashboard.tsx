...
import { Zap, Target, Clock, ShieldCheck, Globe } from "lucide-react";
import { subscribeTickAnalysis, getMtfAnalysis, TickAnalysis, MtfAnalysis } from "@/lib/analysis-service";
...
export default function AiAnalysisDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center p-12 bg-gray-900/20 rounded-3xl border border-gray-800 animate-pulse">
      <p className="text-gray-500 font-medium tracking-widest uppercase text-xs">AI 실시간 수급 분석 중...</p>
    </div>
  );
      ...
      >
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h4 className="font-bold text-white text-base">실시간 수급 모멘텀</h4>
          ...
          <motion.div 
            ...
            className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
              analysis?.signal === 'BUY' ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 
              analysis?.signal === 'SELL' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 
              'bg-gray-500/10 text-gray-500 border border-gray-500/20'
            }`}
          >
            {analysis?.signal === 'BUY' ? '매수 관점' : analysis?.signal === 'SELL' ? '매도 관점' : '관망/중립'}
          </motion.div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="relative h-24 flex items-end gap-1 px-4">
             {barData.map((bar, i) => (
                <motion.div 
                  ...
                  transition={{ repeat: Infinity, repeatType: "reverse", duration: bar.duration }}
          ...
          <div className="flex items-center justify-between">
             <div className="flex flex-col">
                <span className="text-3xl font-black text-white">{analysis?.intensity || 0}%</span>
                <span className="text-[10px] text-gray-500 uppercase font-bold tracking-tighter italic">AI 데이터 신뢰도</span>
             </div>
        ...
        <p className="mt-4 text-[9px] text-gray-600 font-medium leading-tight">
          ※ 본 데이터는 과거 통계와 알고리즘 기반의 <b>학습용 분석 정보</b>이며, 투자의 책임은 본인에게 있습니다.
        </p>
      </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
             ...
             <div>
               <h4 className="font-bold text-white text-base">MTF 매물대 (POC)</h4>
               <p className="text-[10px] text-gray-500 uppercase flex items-center gap-1 font-bold">
           ...
           {mtfData?.analysis.map((tf) => (
             <button
               ...
               className={`flex-1 py-1.5 rounded-lg text-[10px] font-black transition-all ${
                 activeTf === tf.tf ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'
               }`}
             >
               {tf.tf}
             </button>
             ...
             <div key={i} className="flex flex-col gap-2">
                <div className="flex items-center justify-between group cursor-help">
                   <div className="flex items-center gap-2">
                      <span className="text-[10px] font-bold text-gray-600 bg-gray-900 w-5 h-5 flex items-center justify-center rounded-md">{i+1}</span>
                      <span className="text-sm font-black text-gray-200 tracking-tight">${level.price}</span>
                   </div>
                   <span className="text-[10px] font-mono text-blue-400 opacity-70">VOL {level.volume.toLocaleString()}</span>
                </div>
                <div className="h-1.5 w-full bg-gray-900 rounded-full overflow-hidden border border-gray-800/50">
                   <motion.div 
                     initial={{ width: 0 }}
                     animate={{ width: `${(level.volume / currentTfData.levels[0].volume) * 100}%` }}
        ...
        <div className="mt-8 p-4 rounded-2xl bg-blue-500/5 border border-blue-500/10 flex items-start gap-3">
           ...
           <p className="text-[10px] text-blue-300 leading-relaxed font-medium">
             <b>{activeTf} 타임프레임</b> 기준 핵심 POC는 <b>${currentTfData?.poc}</b>입니다. <br />
             장기(D, H1)와 단기(M1, M2) 매물대가 겹치는 구간을 최우선으로 분석하세요.
           </p>
        </div>
      </motion.div>
    </div>
