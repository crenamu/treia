...
export default function EducationalPerspectiveCard({
	levels,
	analysis,
	scenarios,
}: PerspectiveProps) {
					...
					<div className="space-y-4">
						...
						<div className="flex items-start gap-3 p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10">
							...
							<p className="text-xs text-amber-200/80 font-medium leading-relaxed">
								<b>학습 포인트:</b> 전일 저점에서의 강한 회귀 본능은 단순히
								가격의 상승이 아닌 기관의 유동성 매집 프로세스로 이해해야
								합니다.
							</p>
						</div>
					</div>
						...
						{levels.map((level, i) => (
							...
							>
								<div className="flex items-center gap-3">
									<div
										className={`w-1.5 h-1.5 rounded-full ${level.type === "major" ? "bg-cyan-400" : "bg-yellow-500"}`}
									/>
								...
								<span
									className={`text-sm font-black ${level.type === "major" ? "text-cyan-400" : "text-yellow-500"}`}
								>
