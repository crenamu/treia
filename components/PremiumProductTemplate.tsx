			...
			<div className="container mx-auto max-w-2xl text-left">
				...
				<header className="px-6 pt-10 pb-8">
					<div className="flex flex-col md:flex-row items-center md:items-start gap-4 mb-6 text-center md:text-left">
						...
						<div className="flex flex-col items-center md:items-start">
							<span className="text-sm font-bold text-gray-400 mb-1">
								{product.kor_co_nm || product.company}
							</span>
					...
					<div className="grid grid-cols-2 gap-4 py-8 border-y border-gray-50 mt-4">
						<div className="flex flex-col items-center md:items-start">
							<span className="text-[11px] font-black text-gray-300 uppercase tracking-widest mb-2">
								{primaryRate.label}
							</span>
							<span
								className={`text-4xl font-outfit font-black ${theme.primary} tracking-tighter`}
							>
								{primaryRate.value}
								<span className="text-xl ml-0.5">{primaryRate.suffix}</span>
							</span>
						</div>
				...
				{ranking && (
					...
					>
						<div className="flex items-center gap-5 text-left">
							<div
								className={`w-12 h-12 ${theme.bg} rounded-2xl flex items-center justify-center ${theme.primary} shadow-sm`}
							>
								<Trophy size={24} />
							</div>
							<div>
								...
								<p className="text-lg font-black text-gray-900 tracking-tight">
									상위{" "}
									<span className={theme.primary}>TOP {ranking.rank || 5}</span>{" "}
									이내 경쟁력 보유
								</p>
							</div>
						</div>
						<ArrowRight
							size={22}
							className="text-gray-200 group-hover:text-gray-400 group-hover:translate-x-1 transition-all"
						/>
					</section>
				)}
				...
				<section className="py-16">
					<div className="px-6">
						<h2 className="text-xl font-black text-gray-900 mb-8 tracking-tight">
							상품 정보
						</h2>
						...
						>
							<div className="px-8 pt-8 pb-4 flex items-center justify-between border-b border-gray-50">
								<div>
									...
									<p className="text-xs text-gray-400 font-bold mt-1">
										상위 {ranking.rank}위를 차지한 아주 경쟁력 있는 상품입니다.
									</p>
								</div>
								<button
									onClick={() => setIsRankingModalOpen(false)}
							...
							<div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-hide">
								{ranking.topProducts.map((p, i) => (
									...
									>
										<div
											className={`w-8 h-8 rounded-full border-2 flex items-center justify-center text-sm font-black ${i === 0 ? "bg-amber-400 border-amber-400 text-white" : "bg-white border-gray-100 text-gray-300"}`}
										>
											{i + 1}
										</div>
										<div className="flex-1">
											<p className="text-[10px] font-bold text-gray-400">
												{p.kor_co_nm || p.company}
											</p>
											<p className="text-sm font-black text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1">
												{p.fin_prdt_nm || p.name}
											</p>
										</div>
										<div className="text-right">
											<p
												className={`text-lg font-black ${i === 0 ? "text-blue-600" : "text-gray-900"}`}
											>
												{p.effectiveRate || p.bestBenefit || "-"}
												{!p.bestBenefit && "%"}
											</p>
										</div>
									</Link>
					...
					<>
						...
						>
							...
							<div className="flex-1 overflow-y-auto p-8 space-y-10 scrollbar-hide">
								{details
									.filter((d) => d.isText || d.label === "우대 조건")
									.map((d, i) => (
								...
								<div className="p-6 bg-blue-50/50 rounded-3xl border border-blue-100 mb-8">
									...
									<p className="text-xs text-blue-700/70 font-medium leading-relaxed">
										위 데이터는 금융감독원 공시 정보를 바탕으로 제공됩니다.
										정확한 금리 및 조건은 가입 시점에 금융기관 홈페이지를 통해
										다시 한번 확인해 주시기 바랍니다.
									</p>
								</div>
							</div>
...
function InfoRow({
	...
	return (
		<div className="py-7 flex flex-col gap-2">
			...
			<p
				className={`text-sm md:text-base text-gray-500 font-medium leading-relaxed ${isText ? "whitespace-pre-wrap" : ""}`}
			>
				{value}
			</p>
		</div>
