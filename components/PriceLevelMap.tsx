          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-3 w-full z-10">
                <div className={`h-[1px] flex-1 ${level.type === 'major' ? 'bg-cyan-500/40' : 'bg-gray-800 border-t border-dashed'}`} />
                <div className={`flex flex-col items-end shrink-0 py-1 px-2 rounded-lg bg-[#0F1115]/80 backdrop-blur-sm border ${
                  level.type === 'major' ? 'border-cyan-500/30' : 'border-gray-800'
                }`}>
                  <span className={`text-[9px] font-black uppercase tracking-tighter leading-none mb-1 ${level.type === 'major' ? 'text-cyan-400' : 'text-gray-500'}`}>
                    {level.label}
                  </span>
                  <span className={`text-xs font-black leading-none ${level.type === 'major' ? 'text-white' : 'text-gray-400'}`}>
