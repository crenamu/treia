...
export default function AuthModal({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			console.error(error);
			if (error.code === "auth/username-exists")
	...
	const handleGoogleLogin = async () => {
		...
		try {
			...
			setError(
				"구글 로그인 중 오류가 발생했습니다. 팝업이 차단되었는지 확인해 주세요.",
			);
		} finally {
			setLoading(false);
		}
			...
			>
				...
				<div className="p-8 flex flex-col gap-6">
					...
					<div className="relative flex items-center">
						...
						<span className="flex-shrink mx-4 text-[10px] text-gray-600 font-black uppercase tracking-widest leading-none">
							OR EMAIL
						</span>
						<div className="flex-grow border-t border-gray-800"></div>
					</div>
