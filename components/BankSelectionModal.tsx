...
export default function BankSelectionModal({ 
  ...
  const currentBankList = activeTab === 'tier1' ? BANK_LIST_TIER_1 : BANK_LIST_TIER_2
  ...
  const toggleAll = () => {
    ...
    const isAllSelected = currentBankNames.every(name => selectedBanks.includes(name))
    
    if (isAllSelected) {
      onSelectBanks(selectedBanks.filter(name => !currentBankNames.includes(name)))
    } else {
      const newSelected = Array.from(new Set([...selectedBanks, ...currentBankNames]))
      onSelectBanks(newSelected)
    }
  }

  const isAllSelected = currentBankList.map(b => b.name).every(name => selectedBanks.includes(name))
        ...
        <>
          ...
          >
            ...
            <div className="px-8 flex gap-8 border-b border-gray-100 mb-6 shrink-0">
               ...
               >
                 1금융권
                 {activeTab === 'tier1' && <motion.div layoutId="modalTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-900" />}
               </button>
               <button 
               ...
               >
                 2금융권
                 {activeTab === 'tier2' && <motion.div layoutId="modalTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-900" />}
               </button>
            </div>

            {/* List Header */}
            <div className="px-8 flex items-center justify-between mb-4 shrink-0">
               <span className="text-base font-black text-gray-900">{activeTab === 'tier1' ? '1금융권' : '2금융권'}</span>
               <button 
                 onClick={toggleAll}
                 className="flex items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-gray-600 transition-colors"
               >
                  <Check size={14} className={isAllSelected ? 'text-green-500' : ''} />
                  전체 선택
               </button>
            </div>
                  ...
                  {currentBankList.map((bank) => {
                    ...
                    return (
                      ...
                      >
                         ...
                         <div className={`w-12 h-12 rounded-full mb-3 flex items-center justify-center font-black text-xs shadow-sm ${
                           isSelected ? 'bg-amber-400 text-white' : 'bg-gray-50 text-gray-400 border border-gray-100'
                         }`}>
                            {bank.name.substring(0, 2)}
                         </div>
                         <span className={`text-[11px] font-bold text-center leading-tight ${isSelected ? 'text-amber-800' : 'text-gray-500'}`}>
                            {bank.name}
                         </span>
                      </button>
                    )
