...
import type { Product } from "@/app/actions/finance";
...
export default function CompactProductCard({
	...
	return (
		<Link href={`/${type}s/${product.fin_prdt_cd}`} className="block">
			<motion.div
				layout
			...
			>
				...
				<div className="text-right shrink-0 px-4 border-r border-gray-50 mr-2">
					...
					<p className="text-[10px] font-bold text-gray-400">
						기본 {product.bestOption?.intr_rate}%
					</p>
				</div>

				<ChevronRight
					size={20}
					className="text-gray-200 group-hover:text-[#10B981] transition-all group-hover:translate-x-1"
				/>
			</motion.div>
		</Link>
	);
