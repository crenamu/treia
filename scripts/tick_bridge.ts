...
const TICK_FILE_PATH =
	process.env.TICK_DATA_PATH || "C:/Users/Public/treia_gold_ticks.csv";
...
async function analyzeAndStore(ticks: any[]) {
	try {
		...
		if (hour >= 8 && hour < 10)
			sessionPhase = "Phase_A_Initial"; // 아시아 시가
		else if (hour >= 15 && hour < 17)
			sessionPhase = "Phase_Neutral_Gap"; // 유럽 전 휴식기
		else if (hour >= 17 && hour < 19)
			sessionPhase = "Phase_E_Initial"; // 유럽 시가
    ...
    `;
		...
		console.log(
			`🤖 AI [${sessionPhase}] 분석 완료: ${analysisJson.sentiment} (${analysisJson.intensity}%)`,
		);
	} catch (error) {
